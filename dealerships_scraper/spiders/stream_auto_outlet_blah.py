...
class StreamAutoOutletSpider(scrapy.Spider):
  ...
  def parse(self, response):
    print("yo")
    print("hello", response.xpath("//div").extract())
    links = response.xpath("//h4[@class='srp-vehicle-title']/a/@href").extract()
    for link in links:
      url = urljoin(response.url, link)
      yield SeleniumRequest(url=url, wait_time=3, callback=self.parse_car)
      # time.sleep(1)
      # yield scrapy.Request(url, callback=self.parse_car, meta=dict(
			#   playwright = True,
    ...
    if next_page:
      ...
      yield SeleniumRequest(url=next_page_url, wait_time=3, callback=self.parse)
